...
class initGateway:
    ...
    def operatingVersion(self, x):

        self.inputInfo = x

        if self.inputInfo == self.possibleVariants[0]:
        ...
        if self.inputInfo not in self.possibleVariants:
...
ig = initGateway()
print(ig.operatingVersion(ig.introFunc()))
